  ...
  use DebitCreditTrait;  
    ...
    {     
      ...
      {         
        ...
        $data->date = $request->production_date;
        ...
        $production_id = $data->id;
         ...
         for ($i=0; $i < $total_item; $i++) { 
          ...
          //$pd['date'] = $request->date[$i]; // it was for flour mills
          $pd['date'] = $request->production_date; // changed for aysha production
          $pd['product_id'] = $request->product_id[$i];
          $pd['qty'] = $request->qty[$i];
          
          if(!isset($request->unit_qty[$i])){
            $unit_qty = 1;
          }else{
            $unit_qty = $request->unit_qty[$i];
          }
          $pd['unit_qty'] =  $unit_qty;
          $pd['total_qty'] = $unit_qty * $request->qty[$i];
           
          DB::table('production_details')->insert($pd); 
          
          $debit_type = "Production";
          
          $this->debitProductStock($request->product_id[$i], $request->store_id, $request->qty[$i], $production_id, $debit_type);
    ...
    {       
       return redirect('productions')->with(session()->flash('alert-success', 'Update page is under construction yet. Please wait.'));    
        ...
        for ($i=0; $i < $total_item; $i++) { 
          ...
          $pd['product_id'] = $request->product_id[$i];
          $pd['qty'] = $request->qty[$i];
          $pd['unit_qty'] = $request->unit_qty[$i];
          $pd['total_qty'] = $request->unit_qty[$i] * $request->qty[$i];
