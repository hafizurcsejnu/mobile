...
{   
    ...
    {
        $orderData = DB::table('returns')
        ->join('customers', 'returns.customer_id', '=', 'customers.id')       
        ->select('returns.*', 'customers.title as customer', 'customers.mobile as mobile')
        ->where('returns.client_id', session('client_id'))
        ->where('returns.active', 'on')
        ->orderby('returns.id', 'desc')
        ->get();        

        $returns=view('admin.returns')
        ->with('order',$orderData);

        return view('admin.master')
        ->with('main_content',$returns);      
    }
  
    
    public function show($id)
    {
        $order = DB::table('returns')
                ->where('id',$id)
                ->first();
        ...
        $products = DB::table('return_details')
                ->join('products', 'return_details.product_id', '=', 'products.id')
                ->select('return_details.*', 'products.name as product_name')
                ->where('order_id',$id)
                ->get();     
      ...
      {      
        ...
        $order_id = DB::table('returns')->insertGetId($order_data);
      ...
      for ($i=0; $i < $total_item; $i++) { 
          ...
          $return_details['customer_id'] = $customer_id;
          $return_details['order_id'] = $order_id;
          ...
          $return_details['qty'] = $request->qty[$i];
          ...
          $return_details['sub_total'] = $request->price[$i]*$request->qty[$i];             
          ...
          DB::table('return_details')->insert($return_details); 
