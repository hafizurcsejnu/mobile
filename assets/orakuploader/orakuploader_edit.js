(function ( $ ) {
    ...
    $.fn.orakuploader = function( options ) {
		...
		for (i = 0; i < settings.orakuploader_attach_images.length; i++) {  
			...
			$(clone).html("<div class='picture_delete'>"+settings.orakuploader_file_delete_label+"</div><img src='"+image+"' alt='' onerror=this.src='"+settings.orakuploader_path+"/images/no-image.jpg' class='picture_uploaded'/> <input type='hidden' value='"+image+"' name='"+settings.orakuploader_field_name+"[]' />");
			$(clone).attr('id', image);
