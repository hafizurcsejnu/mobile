          ...
          <div class="card-body px-1 px-md-3">                                   
            <div role="main" class="main-content">         
              <div class="page-content container container-plus">
                ...
                <h1 class="page-title text-primary-d2 text-140">Expenses </h1>     
                <a href="add-expense" class="btn btn-white px-3 text-95 radius-round border-2 brc-black-tp10 float-right">
                  ...
                  <table id="datatable" class="d-style w-100 table text-dark-m1 text-95 border-y-1 brc-black-tp11 collapsed">
                    ...
                    <thead class="sticky-nav text-secondary-m1 text-uppercase text-85">
                      <tr>
                          ...
                          <th  class="border-0 bgc-white bgc-h-yellow-l3 shadow-sm"> Expensed By</th>					
                    ...
                    @foreach($data as $item)                                   
                      <tr class="d-style bgc-h-default-l4">
                        ...
                        <td>{{bd($item->date)}}</td> 
                        <td><a title="Expense Details" href="edit-expense/{{$item->id}}" target="_blank"> 
                        @if ($item->expense_type == 'Ex Expense') 
                          @php
                              $equipement = DB::table('products')
                                  ->where('id', $item->equipement_id)
                                  ->first();
                              if($equipement != null){
                          ...
                          @endphp 
                        @endif 
                        {{ucfirst($item->title)}}</a></td>
                        @if ($item->expense_type == 'Product Purchase' || $item->expense_type == 'Advance Payment')
                          <td><a title="Company Ledger" href="company-ledger/{{$item->company_id}}" target="_blank">{{$item->expense_type}}</a></td>
                        @else
                           <td>{{$item->expense_type}}</td>     
                        @endif
                                          
                        <td class="tr">{{number_format($item->amount, 2)}}</td>
                        <td>{{$item->expensed_by}}</td>
                        <td>{{$item->payment_method}}</td>
					 

                        <td class="align-middle">
                          <span class="d-none d-lg-inline">
                              <a  title="Edit" href="edit-expense/{{$item->id}}" class="v-hover">
                                  <i class="fa fa-edit text-blue-m1 text-120"></i>
                              </a>
                          </span>

                          <span class="d-lg-none text-nowrap">
                              <a title="Edit" href="edit-expense/{{$item->id}}" class="btn btn-outline-info shadow-sm px-4 btn-bgc-white">
                              ...
                              <div class="modal-dialog modal-dialog-centered modal-confirm">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-footer">
                                    ...
                                    <a href="" class="btn btn-danger delete-expense">Delete</a>
                                  </div>
                                </div>
                              </div>
                            </div>                              
